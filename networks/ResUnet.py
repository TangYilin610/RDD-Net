...
class ResnetModel(nn.Module):
    ...
    def __init__(self, resnet='resnest50',num_classes=2, pretrained=False, mixstyle_layers=[],random_type=None, p=0.5):
        ...
        self.classifier = nn.Sequential(nn.Linear(20070, 1), nn.Dropout(0.5))#221952,3504384,avg:861184
    ...
    def forward(self, x,is_train):
        ...
        # max_x1 = torch.max(x)
        # print(max_x1)
        # y = self.resnet.conv1(y)  # x.size[4, 64, 320, 320]
        # # print('x')
        # # print('x.size()75',x.size())
        # y = self.resnet.bn1(y)  # x.size()78 torch.Size([4, 64, 320, 320])
        # # print('x')
        # # print('x.size()78',x.size())

        # print('x.relu:',x.size())
        x1 = self.resnet.maxpool(x)  # x1.size()83 torch.Size([4, 64, 160, 160])
        # print('x1.size()83',x1.size())
        # ---- low-level features ----
        # x2 = self.resnet.layer1(x1)  # x2.size()86 torch.Size([4, 256, 160, 160])
        # mix2 = None
        if is_train:
        ...
        # y3_1 = self.conv5_1(x3)
        y3_2 = self.conv5_2(x3)
        # y3_3 = self.conv5_3(x3)
        #
        ...
        # y4_1 = self.conv6_1(x4)
        y4_2 = self.conv6_2(x4)
        # y4_3 = self.conv6_3(x4)
        ...
        y2 = F.interpolate(y2, size=(28, 28), mode='bilinear', align_corners=False)
        #
        ...
        classtoken3_1 = torch.cat([classtoken2, y4], 1)
        # x = self.classifier(F.adaptive_avg_pool2d(image, (1, 1)).squeeze())
        classtoken3 = torch.flatten(image, 1)
